...
namespace uaudio
{
	namespace player
	{
		namespace utils
		{
			...
			float* ToSample(unsigned char* data, size_t buffersize, uint16_t blockAlign, uint16_t channels, size_t numSamples, bool left = true);
			int GetPeak(unsigned char* data, size_t data_size, uint16_t blockAlign, uint16_t channels, int scale, bool left = true);
		}
	}
}